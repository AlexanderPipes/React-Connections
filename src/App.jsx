...
function Board(  ) {
  ...
  function handleClick(i) {
    ...
    if(count>=4 && gameState[i].isActive === false) { 
      ...
      //let nextActive = JSON.parse(JSON.stringify(gameState));dsfsdf
    ...
    <>
      <div className="board-game">
        ...
        <div className = "bttn-row">
          <SubmitButton activeButtons={gameState.filter(item => item.isActive)} onSubClick={() => checkMatch()}/>
          <DeselectButton activeButtons={gameState.filter(item => item.isActive)} onResClick = {() => deselect()}/>
        </div>
      </div>
    </>
  )
